...
docker: release;
	@echo ==============docker build==============
	docker build -t hiruok/msg-pusher:$(RELEASE_VERSION) -f Dockerfile .

.PHONY: sender-docker
sender-docker: dist_dir sender;
	@echo ==============docker build==============
	docker build -t hiruok/msg-pusher-sender:$(RELEASE_VERSION) -f Dockerfile-sender .

.PHONY: receiver-docker
receiver-docker: dist_dir receiver;
	@echo ==============docker build==============
	docker build -t hiruok/msg-pusher-receiver:$(RELEASE_VERSION) -f Dockerfile-receiver .
